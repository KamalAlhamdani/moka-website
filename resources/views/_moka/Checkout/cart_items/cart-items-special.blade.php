@foreach( $getCartSpecialItems as $item)
{{-- {{dd($item)}} --}}
{{-- {{dd($item->pivot->special_product_id)}} --}}
{{-- {{dd($item->name)}} --}}
    <div class="cart-block cart-block-item clearfix" style="padding: 15px 10px;">
        <div class="image">
            ...
            <label type="submit" class="label label-primary"> {{__('_moka_checkout.special_order')}} </label>
        </div>
        ...
        <form id="delete_cart_item_{{$item->id}}" action="{{ route('web.cart.delete', $item->pivot->id) }}" method="post">
            ...
            <input type="hidden" name="id" value="{{$item->pivot->special_product_id}}">
        </form>
